...
int writeset_add(WriteSet* set, void* addr, void* src, size_t size){
    ...
    void** keys = (void**)set->base;
    keys[set->count] = addr;

    WriteEntry* entries = (WriteEntry*)(set->base + WS_SLOTS * sizeof(void*));
...
void** writeset_keys(WriteSet* set){
    ...
    return (void**)set->base;
}

WriteEntry* writeset_values(WriteSet* set){
    if (!set) return nullptr;
    return (WriteEntry*)(set->base + WS_SLOTS * sizeof(void*));
}
...
int readset_add(ReadSet* set, std::atomic<uint64_t>* lock){
    ...
    ReadEntry* entries = (ReadEntry*)set->base;
